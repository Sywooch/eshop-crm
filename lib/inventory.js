//$(document).ready(function() {
	...
	$('button.add_to_order').on('click', function(){
		...
		var id = $(tr).find("input.tovar_id").val()
		var name = $(tr).find("td.name").text();
		var price = $(tr).find("td.price").text();
		var sklad_id = $(tr).find("input.sklad_id").val();
		var sklad_name = $(tr).find("td.sklad_name").text();
	...
	function add_price_row(id,name,sklad_id,sklad_name,price) {
		...
		var s = '<tr class="tovar-row">';		
			...
			s+='<td class="name">'+name+'<input type="hidden" name="tovar_list['+idf.getTime()+'][tovar_id]" class="tovar_id" value="'+id+'" /></td>';
			s+='<td class="sklad_id">'+sklad_name+'<input type="hidden" name="tovar_list['+idf.getTime()+'][sklad_id]" class="sklad_id" value="'+sklad_id+'" /></td>';
			s+='<td class="price">'+price+'</td>';
			s+='<td class="amount"><input type="text" name = "tovar_list['+idf.getTime()+'][amount]" class="form-control amount" value="'+amount+'" /></td>';		
			s+='<td class="sum">'+sum+'</td>';
			s+='<td><button type="button" class="btn btn-default btn-sm" aria-label="Удалить"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></button></td>';
			s+='</tr>';
		 var new_row = $(s);
		 lr.before(new_row);
		 recalc(doc);
	...
	function delete_row() {			
		...
		if (len<=1) {alert('Должна быть хотя бы одна позиция');return false;}
