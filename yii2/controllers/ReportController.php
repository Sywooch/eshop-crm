...
class ReportController extends BaseController {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            ...
            while ($date < $date2) {
                ...
                foreach ($ya_list['counters'] as $ya) {
                    ...
                    if (!empty($model->host) and $ya['site'] != $model->host)
                        continue;
                    ...
                    if ($res['cnt_all'] < 1 and $ya_stat['totals']['visits'] < 10 and $res['costs'] < 1 and $res['clicks'] < 1)
                        continue;
    ...
    public function actionDeliverycost() {
        ...
        if (!empty($model->sender_id))
            $sender[$model->sender_id] = $senders[$model->sender_id];
        else
            $sender = $senders;
        $in_category = null;
        if (!empty($model->category_id))
            $in_category = $model->category_id;
    ...
    public function actionRefused() {
        ...
        if (!empty($model->refused_id))
            $refuse[$model->refused_id] = $refused[$model->refused_id];
        else
            $refuse = $refused;
        $in_category = null;
        if (!empty($model->category_id))
