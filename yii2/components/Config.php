...
class Config extends Component
{
    ...
    {
        return ArrayHelper::map($this->_db->createCommand("SELECT name, value FROM {$this->tableName} WHERE shop_id = '$current_shop'")->queryAll(), 'name', 'value');
    }
    ...
    public function set($name, $value = null)
    {
        if (is_array($name)) {
            ...
            foreach ($name as $key => $val) {
                ...
                $insert[] = [$key, $val, $this->current_shop];
                $delete[] = $key;
            ...
            if (count($insert) > 0) {
                ...
                $this->_db->createCommand()->batchInsert($this->tableName, ['name', 'value', 'shop_id'], $insert)->execute();
            }
        } else {
            $value = $this->_merge($name, $value);

            if (array_key_exists($name, $this->getData()) === false) {
                $this->_db->createCommand()->insert($this->tableName, ['name' => $name, 'value' => $value, 'shop_id' => $this->current_shop])->execute();
            } else {
                $this->_db->createCommand()->update($this->tableName, ['value' => $value, 'shop_id' => $this->current_shop], 'name = :name', [':name' => $name])->execute();
            }
